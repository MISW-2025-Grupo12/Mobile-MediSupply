...
class ClientesRepositoryTest {
    ...
    private fun createPaginatedResponse(clientes: List<ClienteAPI>): PaginatedResponse<ClienteAPI> {
    ...
    fun `getClientes should return success when API call is successful`() = runTest {
        ...
        val paginatedResponse = createPaginatedResponse(testClientes)
        val response = Response.success(paginatedResponse)
        coEvery { mockApiService.getClientes("Bearer $token", 1, 20) } returns response
    ...
    fun `getClientes should return failure when API call returns null body`() = runTest {
        // Given
        val token = "test-token"
        val response = Response.success<PaginatedResponse<ClienteAPI>>(null)
        coEvery { mockApiService.getClientes("Bearer $token", 1, 20) } returns response
    ...
    fun `getClientes should return failure when API call throws exception`() = runTest {
        ...
        coEvery { mockApiService.getClientes("Bearer $token", 1, 20) } throws exception
    ...
    fun `getClientes should format Bearer token correctly`() = runTest {
        ...
        // Then
        coVerify { mockApiService.getClientes("Bearer $token", 1, 20) }
    }
    ...
    fun `getClientes should handle empty token`() = runTest {
        ...
        val paginatedResponse = createPaginatedResponse(testClientes)
        val response = Response.success(paginatedResponse)
        coEvery { mockApiService.getClientes("Bearer ", 1, 20) } returns response
        ...
        // Then
        assertTrue(result.isSuccess)
        coVerify { mockApiService.getClientes("Bearer ", 1, 20) }
    }
    ...
    fun `getClientes should handle successful response with empty list`() = runTest {
        ...
        val paginatedResponse = createPaginatedResponse(emptyList)
        val response = Response.success(paginatedResponse)
        coEvery { mockApiService.getClientes("Bearer $token", 1, 20) } returns response
    ...
    fun `getClientes should handle successful response with single cliente`() = runTest {
        ...
        val paginatedResponse = createPaginatedResponse(singleCliente)
        val response = Response.success(paginatedResponse)
        coEvery { mockApiService.getClientes("Bearer $token", 1, 20) } returns response
    ...
    fun `getClientes should handle timeout exception`() = runTest {
        ...
        coEvery { mockApiService.getClientes("Bearer $token", 1, 20) } throws timeoutException
    ...
    fun `getClientes should handle connection exception`() = runTest {
        ...
        coEvery { mockApiService.getClientes("Bearer $token", 1, 20) } throws connectionException
